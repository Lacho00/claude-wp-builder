# Postmortem — reference bilingual Tailwind build

What one full client build (52 commits, one external web audit, one `.claude/CLAUDE.md`
...
Evidence: `git log` on that project, its uncommitted audit-remediation
diff, and its external audit report (166/244 criteria, 38 failures).

---

## A. Fixed in this branch

| # | Defect | Cost on that build | Root cause | Fix |
|---|---|---|---|---|
...
| A14 | Translating a proper-noun taxonomy | slugs became `<term>-en` in shareable URLs; the importer flipped 14 terms `es`→`en` and emptied every Spanish facet | undocumented | `wp-polylang`: do not register such a taxonomy |
...
| B10 | `:nth-child` margins inside a `gap` grid | one card 33px out of line in every events-archive row | rhythm comes from `gap` |
...
| C3 | The audit's remaining code findings — `srcset`/`loading="lazy"` coverage, 65ch measure, required-field marks, link underlines, `x-default` hreflang, gzip/`Cache-Control` | these belong to the `wp-audit-*` agents, which were never run on that project; verify they are covered there before adding anything |
