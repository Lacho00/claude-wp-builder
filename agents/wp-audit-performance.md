...
| PERF-053 | Oversized theme image asset | Glob `assets/**` for `*.webp`, `*.png`, `*.jpg`. Flag any file over 100KB, and any file over 40KB whose `magick <file> -format '%[fx:standard_deviation]' info:` is under 0.12 (flat art: gradient, glass panel, solid shape) | WARNING | Yes |
